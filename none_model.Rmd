...
```{r}
anova(final_model,none_model_full)
```
...
ggplot(data=full_dat, aes(x=mABC_ManualDexterity.Component.StandardScore,y=SRS_TotalRawScore,col=as.factor(SRS_VERSION))) + 
  ...
  geom_abline(slope=final_model$coefficients[2]+final_model$coefficients[3],
              intercept=final_model$coefficients[1], col='red') +
  geom_abline(slope=final_model$coefficients[2]+final_model$coefficients[3],
              intercept=final_model$coefficients[1]+final_model$coefficients[4], col='blue') + 
...
$$
y_i = \beta_0 + d_1\alpha_1 + \beta_1 x_1 + \gamma d_1 x_1 \\
d_1 = \begin{cases} 
      1 & version = 2 \\
      0 & version = 1
   \end{cases}
